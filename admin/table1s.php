...
<div class="col-md-7 text-center heading-section ftco-animate">
    <center><a target='_blank' href='excel/table1countfilter.php?awal=<?php echo $awal; ?>&akhir=<?php echo $akhir; ?>&floor=<?php echo $floor; ?>'>DOWNLOAD EXCEL</a></center>

  <h2 align="center"><strong>Data Survey Lantai <?php echo $floor?></strong></h2><br />
    <form action="table1s.php?floor=<?php echo $floor?>" method="post" name="postform">

      <table border="0" align="center">
          <tr>
              <td width="250"><b>Dari Tanggal</b></td>
              <td width="250">: <input type="date" name="awal" size="16" required value="<?php echo $awal; ?>"/>
              </td>
              <td width="250"><b>Sampai Tanggal</b></td>
              <td width="250">: <input type="date" name="akhir" size="16" required value="<?php echo $akhir; ?>"/>
              </td>
    ...
    <table class="zebra-table" width="1100" border="1">
        ...
        echo $buff ['text'];?></td>
    		<td width="100"><?php
        $sql = "SELECT COUNT(q1) AS jumlah FROM `data` WHERE q1='tidak puas' && floor='$floor' && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
        <td width="100"><?php
        $sql = "SELECT COUNT(q1) AS jumlah FROM `data` WHERE q1='kurang puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>

    		<td width="100"><?php
        $sql = "SELECT COUNT(q1) AS jumlah FROM `data` WHERE q1='puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
    		<td width="100"><?php
        $sql = "SELECT COUNT(q1) AS jumlah FROM `data` WHERE q1='sangat puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
    	</tr>
      <tr>
        ...
        echo $buff ['text'];?></td>
    		<td width="100"><?php
        $sql = "SELECT COUNT(q2) AS jumlah FROM `data` WHERE q2='tidak puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
    		<td width="100"><?php
        $sql = "SELECT COUNT(q2) AS jumlah FROM `data` WHERE q2='kurang puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>

    		<td width="100"><?php
        $sql = "SELECT COUNT(q2) AS jumlah FROM `data` WHERE q2='puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
    		<td width="100"><?php
        $sql = "SELECT COUNT(q2) AS jumlah FROM `data` WHERE q2='sangat puas' && floor='$floor'  && time between '$_POST[awal]' and '$_POST[akhir]'";
        $query = mysqli_query($con,$sql);
        $result = mysqli_fetch_array($query);
        echo "{$result['jumlah']}";?></td>
    	</tr>
  ...
  <table class="zebra-table" width="1100" border="1">
    ...
    <tbody class="huruf">
      <?php
      $pilih="select * from data where floor='$floor' && saran!=''  && time between '$_POST[awal]' and '$_POST[akhir]'";
      $hasil=mysqli_query($con,$pilih);
while($buff=mysqli_fetch_array($hasil)){
      ?>
    <tr>
    		<td width="30"><?php echo $buff ['id_data'];?></td>
    		<td width="70"><?php echo $buff ['time'];?></td>
